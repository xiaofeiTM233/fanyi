...
# 输入Cookie
cookies = input(f'请输入Cookie（留空不登录）：') or '0'
...
if cookies == '0':
    ...
    if cookies == '1':
        ...
        useTerm = input(f'是否使用术语库（留空不使用）：') or '0'
        if useTerm == '0':
            useTerm = 'false'
        else:
            useTerm = 'true'
...
def get_from():
    ...
    # 判断是否报错
    try:
        from_data = int(input(f'{dic_code}：') or '0')
...
word = input('请输入语句') or '当サイトはYostarのブルーアーカイブ（ブルアカ）の攻略情報をまとめた有志wikiとなります。'
